...
- Mosquitto：`logs\mqtt.log` にエラー、およびクライアントの接続・切断（`New client connected`/`Client <id> disconnected`など）を出力（`config\mosquitto.conf` の `log_type error` / `log_type notice` 設定による）
